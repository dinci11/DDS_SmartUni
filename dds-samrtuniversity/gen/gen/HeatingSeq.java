...
/**
* A sequence of Heating instances.
*/
public final class HeatingSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public HeatingSeq (int initialMaximum) {
        super(Heating.class, initialMaximum);
    }

    public HeatingSeq (Collection<?> elements) {
        super(Heating.class, elements);
    }

    public  Heating get(int index) { 
        return (Heating) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Heating.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Heating objects (beyond the original size of this object)
