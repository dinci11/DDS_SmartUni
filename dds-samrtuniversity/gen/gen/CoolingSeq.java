...
/**
* A sequence of Cooling instances.
*/
public final class CoolingSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CoolingSeq (int initialMaximum) {
        super(Cooling.class, initialMaximum);
    }

    public CoolingSeq (Collection<?> elements) {
        super(Cooling.class, elements);
    }

    public  Cooling get(int index) { 
        return (Cooling) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Cooling.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Cooling objects (beyond the original size of this object)
